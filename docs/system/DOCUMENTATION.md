...
Цей документ синхронізований зі станом коду. Після кожної зміни — оновлюємо. ОСТАННЄ ОНОВЛЕННЯ: 06.09.2025 23:00
...
- **Підтримка типів повідомлень**: Повна підтримка всіх типів TalkyTimes повідомлень:
 ...
 - **UI/UX чату** — ДОДАНО 12.09.2025:
  - **Квадратна кнопка відправки**: 40x40px з іконкою літака замість тексту "Надіслати"
  - **Автофокус**: інпут поле автоматично активується при переході в чат (через 100мс)
  - **Обмеження символів**: максимум 300 символів з візуальним лічильником над інпутом
  - **Колірна індикація**: сірий→жовтий→червоний при наближенні до ліміту (200/250 символів)
 - **Видалення зі "Вхідні" після відправки** — ДОДАНО 06.09.2025:
  - **Миттєве видалення**: Після успішної відправки будь-якого типу повідомлення (текст, стікер, фото/відео/аудіо, подарунок, лист, пост) діалог одразу видаляється зі списку "Вхідні"
  - **Селективна логіка для листів**: При відправці листа видаляється лише айтем з поміткою "Новий лист", чат-айтем залишається
  - **Для чат-повідомлень**: Видаляється звичайний чат-айтем, але email-айтем (якщо є) зберігається
  - **Реалізація через події**: Використовується `window.dispatchEvent('dialog:sent')` з параметрами `profileId`, `clientId`, `kind`
  - **Слухач у `layout.tsx`**: Миттєво фільтрує список діалогів без перезавантаження з сервера
 - **Автооновлення списку "Вхідні"** — ДОДАНО 06.09.2025:
  - **Періодичне оновлення**: Кожні 60 секунд автоматично перезавантажується список діалогів у фільтрі "Вхідні"
  - **Умовне виконання**: Активується тільки коли вибраний фільтр "Вхідні" (`status: 'unanswered'`)
  - **Повне перезавантаження**: Використовується `loadDialogs(filters, true)` для отримання актуальних даних
  - **Автоматичне очищення**: Інтервал очищується при зміні фільтра або unmount компонента
  - **Автоматичне розширення**: textarea динамічно збільшується від 48px до 120px при потребі
  - **Клавіатурні скорочення**: Enter відправляє, Shift+Enter новий рядок
...
#### 🎨 UI/UX
- **Кастомні елементи управління** — ДОДАНО 11.09.2025:
  - **CustomSelect**: мінімалістичний кастомний випадаючий список замість системного `<select>`
    - Компонент: `apps/web/src/components/CustomSelect.tsx`
    - Особливості: плавні анімації, клавіатурна навігація, закриття при кліку поза елементом
    - Використання: фільтр діалогів, пошук профілів у чатах
  - **Кастомні скролбари**: заміна системних скролбарів на мінімалістичні
    - Стилі: `apps/web/src/app/globals.css` (.custom-scroll клас)
    - Ширина: 6px, колір: сірий з hover ефектами
    - Підтримка: WebKit (Chrome/Safari) та Firefox
  - **Emoji Picker**: вибір емодзі для всіх текстових полів
    - Компонент: `apps/web/src/components/EmojiPicker.tsx`
    - Особливості: вспливаюче вікно з сіткою 5xN популярних емодзі, фіксовані слоти 36x36px
    - Інтеграція: чат повідомлення, ексклюзивні пости, повідомлення до подарунків, листи
    - UX: кнопка 😀 поруч з інпутом, додавання кількох емодзі без закриття, закриття тільки Escape або клік поза межами
...
- **Email-айтеми у списку діалогів**:
  - Відзначаються синім бейджем "Новий лист" з іконкою листа
  - Розташування: дата зверху, бейдж знизу з автоматичним відступом між ними
  - Стиль: жирний текст (`font-bold`), іконка листа 12x12px, синій колір (`text-blue-600`)
  - Вирівнювання: по правому краю з мінімальною висотою контейнера
