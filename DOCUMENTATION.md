...
**Підключення до RTM:**
1. Отримання активної сесії з БД (`TalkyTimesSession`)
2. Валідація сесії (перевірка `tld-token` та терміну дії)
3. Підключення до `wss://talkytimes.com/rtm` з cookies
4. Автоматична підписка на `personal:#<userId>` канал
5. Обробка повідомлень:
   - `MessageSent`: нові повідомлення
   - `chat_DialogLimitChanged`: оновлення лімітів

**Toast сповіщення:**
1. Отримання RTM повідомлення
2. Створення toast з `dialogId` для навігації
3. Перевірка поточної сторінки перед навігацією
4. Відображення toast з текстом повідомлення
5. Навігація до діалогу при кліку (якщо не відкритий)

**Оптимізації RTM:**
- Єдиний метод підключення з динамічними cookies
- Автоматичне перепідключення (до 3 спроб)
- Таймаут підключення 10 секунд
